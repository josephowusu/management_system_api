...
import { iDebtCategoryTable, iDebtCategoryUpdatableColumns } from "../../../modules/interfaces/miniAccount/iDebtCategoryTable"


class DebtCategoryTable {

    private dataObject:iDebtCategoryTable
    private database:MySQL.Connection|null|undefined
    private schema:string

    constructor(data?:iDebtCategoryTable, database?:MySQL.Connection|null, schema?:string) {
        this.dataObject = data ? data : { id: generateID(), status: 'active' }
        this.database = database
        this.schema = schema ? schema : ''
    }

    public setValues(data:iDebtCategoryTable) {
    ...
    public async save() {
        try {
            ...
            let result:any = await dbQuery(`
                INSERT INTO ${this.schema}.debtCategory 
            ...
            if (result && result.affectedRows) {
                ...
                Logger.log('error', 'debtCategoryTable.ts', JSON.stringify(result))
                return {type: 'error'}
            }
        } catch (error:any) {
            Logger.log('error', 'debtCategoryTable.ts', error)
            return {type: 'error'}
        }
    }

    public async update(columnsToUpdate:iDebtCategoryUpdatableColumns[]) {
        try {
            const sqlStatement = getUpdateSqlStatement(this.dataObject, columnsToUpdate)
            let sql = `UPDATE ${this.schema}.debtCategory ${sqlStatement.sql} WHERE id = ? AND status = ?`
    ...
    public async get(search:string, values:any[], limit:number, offset:number) {
        try {
            let result = await dbQuery(`SELECT * FROM ${this.schema}.debtCategory WHERE ${search} LIMIT ${limit} OFFSET ${offset}`, values, this.database)
    ...
    public async getAll(limit:number, offset:number) {
        try {
            let result = await dbQuery(`SELECT * FROM ${this.schema}.debtCategory LIMIT ${limit} OFFSET ${offset}`, [], this.database)
            return Array.isArray(result) ? result : []
        } catch (error:any) {
            Logger.log('error', 'debtCategoryTable.ts', error)
            return []
...
export default DebtCategoryTable