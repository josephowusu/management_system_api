...
import { iTierData, iTierTableUpdatableColumns } from "../../../modules/interfaces/hr/iTierTable"

class TierTable {

    private dataObject:iTierData
    private database:MySQL.Connection|null|undefined
    private schema:string

    constructor(data?:iTierData, database?:MySQL.Connection|null, schema?:string) {
        this.dataObject = data ? data : { id: generateID(), status: 'active' }
        this.database = database
        this.schema = schema ? schema : ''
    }

    public setValues(data:iTierData) {
    ...
    public async save() {
        try {
            ...
            let result:any = await dbQuery(`
                INSERT INTO ${this.schema}.tier
                (id, name, percentage, description, status, sessionID, createdAt) 
                VALUES (?,?,?,?,?,?,?)
            `, [
                this.dataObject.id, this.dataObject.name, this.dataObject.percentage,this.dataObject.description, 
            ...
            if (result && result.affectedRows) {
                ...
                Logger.log('error', 'TierTable.ts', JSON.stringify(result))
                return {type: 'error'}
            }
        } catch (error:any) {
            Logger.log('error', 'TierTable.ts', error)
            return {type: 'error'}
        }
    }

    public async update(columnsToUpdate:iTierTableUpdatableColumns[]) {
        try {
            const sqlStatement = getUpdateSqlStatement(this.dataObject, columnsToUpdate)
            let sql = `UPDATE ${this.schema}.tier ${sqlStatement.sql} WHERE id = ? AND status = ?`
    ...
    public async get(search:string, values:any[], limit:number, offset:number) {
        try {
            let result = await dbQuery(`SELECT * FROM ${this.schema}.tier WHERE ${search} LIMIT ${limit} OFFSET ${offset}`, values, this.database)
    ...
    public async getAll(limit:number, offset:number) {
        try {
            let result = await dbQuery(`SELECT * FROM ${this.schema}.tier LIMIT ${limit} OFFSET ${offset}`, [], this.database)
            return Array.isArray(result) ? result : []
        } catch (error:any) {
            Logger.log('error', 'TierTable.ts', error)
            return []
    ...
    public tableDescription():iGetCreateTableDetails {
        return {
            ...
            columns: [
                ...
                {columnName: 'percentage', dataType: 'double(20, 2)'},
