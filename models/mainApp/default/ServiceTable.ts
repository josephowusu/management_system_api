...
import { iService, iServiceUpdatableColumns } from "../../../modules/interfaces/default/iServices"


class ServiceTable {

    private dataObject:iService
    private database:MySQL.Connection|null|undefined
    private schema:string

    constructor(data?:iService, database?:MySQL.Connection|null, schema?:string) {
        this.dataObject = data ? data : { id: generateID(), status: 'active' }
        this.database = database
        this.schema = schema ? schema : ''
    }

    public setValues(data:iService) {
    ...
    public async update(columnsToUpdate:iServiceUpdatableColumns[]) {
        try {
